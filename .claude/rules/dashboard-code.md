...
- Type interfaces for kit/theme data live in `app/page.tsx` and are imported from there
...
## API Routes
- Kit/theme file serving goes through API routes in `app/api/`
...
## TypeScript
- Strict types for all kit/theme data — use the interfaces from `page.tsx`
- No `any` types — use `Record<string, T>` or proper interfaces
- Props must be typed inline or with a named interface

## Data Flow
- `page.tsx` reads `kit.json` / `theme.json` from disk at build time (server component)
...
## Adding New Entities
When adding a new browsable entity type (beyond kits and themes):
