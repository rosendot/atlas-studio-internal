...
- Type interfaces for kit/section/template/palette data live in `app/page.tsx` and are imported from there
...
## TypeScript
- Strict types for all kit/section/template/palette data — use the interfaces from `page.tsx`
- No `any` types — use `Record<string, T>` or proper interfaces
- Props must be typed inline or with a named interface

## Data Flow
- `page.tsx` reads `kit.json` / `section.json` / `template.json` / `palette.json` from disk at build time (server component)
