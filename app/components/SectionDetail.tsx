...
export default function SectionDetail({
  ...
  const allFiles = Object.entries(section.files).flatMap(([lang, files]) =>
  ...
  // Build preview props: content variables + palette colors/fonts
    ...
    <div className="min-h-[calc(100vh-57px)]">
            ...
            if (!PreviewComponent) {
              return (
                <div className="flex items-center justify-center h-full text-[var(--muted)]">
                  <p>No preview component. Add one to <code className="text-xs bg-[var(--card-bg)] px-1.5 py-0.5 rounded">section-previews/</code></p>
                </div>
              );
            }
      ...
      {activeView === "code" && (
        <div className="flex h-[calc(100vh-114px)]">
            ...
            <div className="flex-1 p-4 overflow-auto">
              <pre className="text-sm font-mono leading-relaxed">
                <code className="text-[var(--foreground)]">
                  {section.fileContents[activeFile] || "// No content available"}
                </code>
              </pre>
            </div>
          </div>
          ...
          <div className="w-72 border-l border-[var(--card-border)] p-5 overflow-auto">
            <div className="mb-5">
              <h3 className="text-xs uppercase tracking-wider text-[var(--muted)] mb-2">Layout</h3>
              <span className="text-xs text-[var(--foreground)] bg-[var(--card-bg)] border border-[var(--card-border)] px-2 py-0.5 rounded capitalize">
                {section.layout}
              </span>
            </div>

            {section.kits_used.length > 0 && (
              <div className="mb-5">
                <h3 className="text-xs uppercase tracking-wider text-[var(--muted)] mb-2">Kits Used</h3>
                <div className="flex flex-col gap-1.5">
                  {section.kits_used.map((kit) => (
                    <span key={kit} className="text-xs text-[var(--accent)] bg-[var(--accent)]/10 px-2 py-1 rounded">{kit}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="mb-5">
              <h3 className="text-xs uppercase tracking-wider text-[var(--muted)] mb-2">Palette</h3>
              <span className="text-xs text-[var(--gold)] bg-[var(--gold)]/10 px-2 py-0.5 rounded-full">
                {activePalette?.name || section.default_palette}
              </span>
            </div>

            <div className="mb-5">
              <h3 className="text-xs uppercase tracking-wider text-[var(--muted)] mb-2">Tags</h3>
              <div className="flex flex-wrap gap-1.5">
                {section.tags.map((tag) => (
                  <span key={tag} className="text-xs text-[var(--gold)] bg-[var(--gold)]/10 px-2 py-0.5 rounded-full">{tag}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      {activeView === "variables" && (
        <div className="max-w-2xl mx-auto p-8">
          <h3 className="text-lg font-semibold mb-1">Section Variables</h3>
          <p className="text-sm text-[var(--muted)] mb-6">
            Content and layout values. Colors come from the palette.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(section.variables).map(([key, variable]) => (
              <div key={key} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-4">
                <label className="text-xs text-[var(--muted)] block mb-2">{variable.label}</label>
                <input
                  type={variable.type === "number" ? "number" : "text"}
                  value={variables[key]}
                  onChange={(e) => handleVariableChange(key, variable.type === "number" ? Number(e.target.value) : e.target.value)}
